...
Version 6 retains the complete v5 product-family, editorial, visibility and Visual Asset System and adds **OpenAI as a first-class optional image-generation provider** behind the existing provider-neutral visual-generation boundary.

- Implements an **OpenAI Visual Generation Provider** as an approved server-side adapter without coupling framework/product records or the admin workflow to OpenAI-specific schemas.
- Uses the current OpenAI image-generation stack at implementation time; as of 13 August 2026, the recommended API image model is **GPT Image 2 (`gpt-image-2`)**, while the configured model remains environment-driven so it can be changed without a database migration.
- Uses the OpenAI **Images API** for direct image generation/editing unless current official OpenAI documentation at build time recommends a different supported endpoint for the required operation.
- Adds an admin **Provider** selector/status display so authorised users can choose OpenAI when enabled, while upload and deterministic-render paths continue to work when no external provider is configured.
- Adds explicit OpenAI server-only configuration, model allow-listing, timeout/retry handling, provider error mapping, rate limits and configurable generation budgets.
- Adds generation-job records so one request, its selected provider/model, candidate count, latency, failure state and safe usage/cost metadata can be audited separately from individual Visual Asset candidates.
- Adds an OpenAI-specific prompt builder that composes the structured Visual Brief + approved Visual Recipe into the provider request; ordinary Editors do not manage API keys or raw provider configuration.
- Supports optional **reference-image/edit workflows** through the same provider adapter for future consistency work, while keeping exact diagrams, Template previews and Tool previews deterministic wherever possible.
- Requires generated image bytes to be copied into IncyTemplates-controlled storage; public pages never depend on temporary provider URLs or live OpenAI availability.
- Makes provider/model changes non-destructive: existing approved visuals remain unchanged until a new generated candidate is explicitly selected, approved and published.
- Adds OpenAI-aware security/privacy rules, environment validation, observability, tests, launch checks and implementation guidance.
- Adds official OpenAI image-generation/model/pricing documentation to the technical references and instructs the coding agent to verify current model IDs, parameters, pricing and deprecation status at implementation time.
- Retains v5 Visual Recipes, Visual Briefs, generated/uploaded/rendered source types, real Template/Tool previews, responsive derivatives and human approval.
- Retains all v4 Guide → Template → Tool behaviour, A Bit Gamey provenance, commerce architecture, content revisions and Public/Unlisted/Hidden controls.
...
**Version 6 retains the v5 product model, editorial controls and governed Visual Asset System and adds OpenAI as a supported optional image-generation provider behind a provider-neutral service boundary.** Incy Templates is not primarily a digital-template store. It is a practical product-development platform that turns reusable methods into three primary output types:
...
9. Admin selects a candidate, completes alt-text/accuracy/brand review, approves it and publishes it through the existing v6 workflow.
...
- Use the configured OpenAI image model, with `gpt-image-2` as the v6 recommended default at this specification date.
...
Repository-managed Markdown may remain useful for seed content, migrations, review exports and backups. However, routine published Guide/Template/Tool copy must be editable through the admin content model defined in v6, so production must not depend on a developer editing repository files for ordinary editorial changes.
...
- `provider_model` is the configured model used for the request. As of v6 the recommended default is `gpt-image-2`, but the table does not enforce that value.
...
20. Editor opens a framework Visuals tab, selects **OpenAI** in a mocked provider environment, creates a brief, generates a bounded candidate set, selects and approves one, and sees the public card/family page update. Ordinary CI must not require a live OpenAI call.
...
- `OPENAI_IMAGE_MODEL` must be validated against a server-side allow-list. `gpt-image-2` is the v6 default, not a permanent invariant.
...
- [ ] Configured OpenAI image model has been checked against current official documentation and is not deprecated; v6 working default is `gpt-image-2`
...
10. Visual provider abstraction and provider registry
11. `it_visual_generation_jobs` lifecycle + budget/rate guard
12. OpenAI provider adapter using current official OpenAI image-generation API, plus mocked test provider
...
25. ~~Select the production image-generation provider.~~ **Resolved for v6:** implement **OpenAI** as an initial supported provider while retaining the provider-neutral architecture and upload/render fallback.
