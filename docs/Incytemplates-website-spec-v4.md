...
Version 4 retains the v3 product-family model and adds a practical browser-based content-management capability for day-to-day operation of the site.

- Adds an **admin content editor** so authorised Editors/Admins can manually update Guide content, Template copy/instructions and Tool-facing text without editing source files or redeploying code.
- Adds **admin Template file replacement/versioning** for downloadable artefacts, with validation and audit history.
- Adds explicit **public visibility controls** for individual Guides, Templates and Tools: `public`, `unlisted` and `hidden`.
- Defines `hidden` as unavailable to ordinary site visitors, including direct public URLs, catalogue/search results, family output lists and sitemaps.
- Adds draft/published content revisions, change notes, rollback and preview so manual edits do not overwrite the currently published version accidentally.
- Keeps executable Tool logic, schemas and deterministic scoring/calculation code in version control; admin editing is limited to approved content and safe configuration.
- Extends admin routes, RLS, caching/revalidation, tests, implementation phases, launch checks and acceptance criteria for content editing and visibility management.
- Retains the v3 Guide → Template → Tool product model, A Bit Gamey provenance, flagship portfolio and commerce architecture.
...
**Version 4 retains the v3 product model and makes it operable without routine code changes.** Incy Templates is not primarily a digital-template store. It is a practical product-development platform that turns reusable methods into three primary output types:
...
Repository-managed Markdown may remain useful for seed content, migrations, review exports and backups. However, routine published Guide/Template/Tool copy must be editable through the admin content model defined in v4, so production must not depend on a developer editing repository files for ordinary editorial changes.
...
Retain v2 Support / Editor / Admin / Owner role separation. Extend Editor/Admin capabilities to frameworks, source links, product-content revisions, Template file versions, Tool presentation configuration and visibility controls. Editors must not gain customer-data access merely because they can edit product content.

Recommended content permissions:

- **Support:** no product-content editing or visibility changes.
- **Editor:** create/edit drafts, preview, publish approved editorial revisions and change `public/unlisted/hidden` visibility for product outputs; no executable Tool changes and no customer-data access.
- **Admin:** Editor capabilities plus product/file/version administration, commerce configuration within approved controls and operational recovery actions.
...
- Revalidation after admin publication, rollback, Template file-version change and public-visibility change.
