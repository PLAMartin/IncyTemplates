...
Version 7 retains the complete v6 product-family, editorial, visibility, commerce and governed Visual Asset/OpenAI architecture and adds an explicit **A Bit Gamey post → IncyTemplates use taxonomy and human-review workflow**.

- Defines **Reuse Taxonomy v1** for assessing every A Bit Gamey source post independently of the archive's existing 16-category subject taxonomy.
- Classifies each post across six reusable dimensions: **Problem, Stage, User task, Method, Frequency and Judgement level**.
- Adds a five-factor **0–10 reuse score**: Problem clarity, Actionability, Repeatability, Structure and Automation potential, each scored 0–2.
- Converts the taxonomy/score into one or more suggested IncyTemplates uses: **Source-only/editorial, Guide, Template and Tool**. Suggestions are advisory and may identify one or more candidate framework/product families.
- Makes Guide suggestion logic explicit rather than treating Guide as a score threshold: a post can justify a Guide because it contains a reusable explanation or method even where Template/Tool structure is weak.
- Adds an admin **Source Post Mapping** review queue where authorised users can see the suggested taxonomy, score, use(s), framework mapping, confidence and rationale, then accept, edit, add/remove mappings or dismiss the suggestion.
- Preserves the original machine/seed suggestion separately from the human editorial decision so later re-analysis never silently overwrites an accepted mapping.
- Extends source-provenance data with versioned post-use assessments, review decisions and mapping origin/output-use metadata.
- Adds source-mapping scripts/service boundaries, admin routes, tests, analytics/portfolio measures, implementation-phase work and launch/acceptance checks.
- Keeps the A Bit Gamey repository an editorial/build-time source only; no public request depends on private GitHub access and no suggested mapping is automatically published.
- Retains v6 OpenAI visual generation as an optional server-side provider, with the current model remaining configuration-driven and subject to verification against current official documentation at implementation time.
- Retains all v6 Guide → Template → Tool behaviour, content revisions, Public/Unlisted/Hidden controls, human approval, commerce, saved work and visual governance.
...
**Version 7 retains the v6 product model, editorial controls and governed Visual Asset/OpenAI System and adds a versioned A Bit Gamey source-post reuse taxonomy with suggested mappings and explicit human review/override.** Incy Templates is not primarily a digital-template store. It is a practical product-development platform that turns reusable methods into three primary output types:
...
Version 7 adds a separate **Reuse Taxonomy v1** whose purpose is not to describe what a post is *about*, but to assess **how the post could be used on IncyTemplates**.
...
9. Admin selects a candidate, completes alt-text/accuracy/brand review, approves it and publishes it through the existing v7 workflow.
...
- Use the configured OpenAI image model, with `gpt-image-2` as the v7 recommended default at this specification date.
...
Repository-managed Markdown may remain useful for seed content, migrations, review exports and backups. However, routine published Guide/Template/Tool copy must be editable through the admin content model defined in v7, so production must not depend on a developer editing repository files for ordinary editorial changes.
...
- `provider_model` is the configured model used for the request. As of v7 the recommended default is `gpt-image-2`, but the table does not enforce that value.
...
Additional v7 source-mapping indexes should include:
...
10. Admin opens the source-post mapping queue, reviews a suggested Guide/Template/Tool mapping, adjusts one mapping, accepts it and sees the original suggestion preserved.
11. Admin marks another post Source-only/dismissed and verifies no framework/product is auto-published.
12. A new assessment version can be created for a reviewed post without silently changing its accepted framework links.
13. Editor changes a Guide body, saves a draft, previews it, publishes it and sees the public page update without a deployment.
14. Editor changes Template instructions and uploads a replacement file as a new version without destroying the previous version.
15. Editor changes Tool-facing help/result copy while the deployed `tool_key` logic remains unchanged.
16. Admin changes an output Public → Hidden; it disappears from catalogue/search/family/sitemap and its ordinary direct public URL no longer renders it; restoring Public makes it visible again.
17. Admin changes an output Public → Unlisted; direct URL remains available but discovery and indexing surfaces omit it.
18. Unauthorised user cannot view another user's Tool run or paid resource.
19. Unauthorised user cannot access admin draft revisions or preview routes.
20. Keyboard-only Tool completion.
21. Mobile Tool flow and result view.
22. Consent preferences and contact form.
23. Editor opens a framework Visuals tab, selects **OpenAI** in a mocked provider environment, creates a brief, generates a bounded candidate set, selects and approves one, and sees the public card/family page update. Ordinary CI must not require a live OpenAI call.
24. Ordinary visitor cannot access rejected/candidate visual files or admin prompt metadata.
25. Editor restores a previous approved visual and the public page revalidates without destroying the newer asset history.
26. A Template preview uses the real rendered/current Template example where the renderer is supported.
...
- `OPENAI_IMAGE_MODEL` must be validated against a server-side allow-list. `gpt-image-2` is the v7 default, not a permanent invariant.
...
- [ ] Configured OpenAI image model has been checked against current official documentation and is not deprecated; v7 working default is `gpt-image-2`
...
4. Product-content revision schema and publication service
5. Public-visibility enum/query rules
6. Public framework/output queries
7. Admin source-mapping review permissions/workflow
8. Admin content preview/publish permission model
9. Tool registry contract
10. Tool-run privacy/ownership model
11. Visual Asset/Recipe schema and private/public storage rules
12. Visual provider abstraction and provider registry
13. `it_visual_generation_jobs` lifecycle + budget/rate guard
14. OpenAI provider adapter using current official OpenAI image-generation API, plus mocked test provider
...
25. ~~Select the production image-generation provider.~~ **Resolved in v6 and retained in v7:** implement **OpenAI** as an initial supported provider while retaining the provider-neutral architecture and upload/render fallback.
