...
// generalised to the third Tool), including its 8-step flow (4 questions x 2 options) —
// the longest of the three Tools so far.
...
test("anonymous visitor can complete the Expected Value Comparator Tool and see a result", async ({ page }) => {
  await page.goto("/tools/better-decision-maker");
  await expect(page.getByRole("heading", { name: "Expected Value Comparator", exact: false }).first()).toBeVisible();
  ...
  // First option in every step is the weakest/most-cautious choice for both options
  // (identical inputs on each side), so this run should land on a tie.
  await expect(page.getByText("Too close to call on numbers alone")).toBeVisible();
...
test("keyboard-only visitor can complete the Expected Value Comparator Tool", async ({ page }) => {
  await page.goto("/tools/better-decision-maker");

  // Tab to and activate "Start the comparison".
  await page.getByRole("button", { name: "Start the comparison" }).focus();
...
test.describe("mobile viewport", () => {
  ...
  test("Expected Value Comparator Tool is completable on a mobile viewport", async ({ page }) => {
    await page.goto("/tools/better-decision-maker");
    await page.getByRole("button", { name: "Start the comparison" }).click();
