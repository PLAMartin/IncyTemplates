...
// Spec §32.3 flow #4 (complete Product Idea Assessor anonymously) and flows #11/#12
// (keyboard-only completion, mobile flow).
...
test("anonymous visitor can complete the Product Idea Assessor Tool and see a result", async ({ page }) => {
  await page.goto("/tools/product-idea-assessor");
  await expect(page.getByRole("heading", { name: "Product Idea Assessor", exact: false }).first()).toBeVisible();
  ...
  await expect(page.getByText(/Evidence quality score/)).toBeVisible();
...
test("keyboard-only visitor can complete the Product Idea Assessor Tool", async ({ page }) => {
  await page.goto("/tools/product-idea-assessor");

  // Tab to and activate "Start the assessment".
  await page.getByRole("button", { name: "Start the assessment" }).focus();
...
test.describe("mobile viewport", () => {
  ...
  test("Product Idea Assessor Tool is completable on a mobile viewport", async ({ page }) => {
    await page.goto("/tools/product-idea-assessor");
    await page.getByRole("button", { name: "Start the assessment" }).click();
