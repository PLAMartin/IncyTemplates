...
// Mirrors tests/e2e/first-customers-planner-tool.spec.ts's coverage generalised to the
// seventh Tool, including its 5-step flow (five rating questions, no classification step).
...
test("anonymous visitor can complete the PMF Score Checker Tool and see a gated result", async ({ page }) => {
  await page.goto("/tools/product-market-fit-tracker");
  await expect(page.getByRole("heading", { name: "PMF Score Checker", exact: false }).first()).toBeVisible();

  await page.getByRole("button", { name: "Start checking fit" }).click();
  ...
  // First option in every step is "Low" for all five ratings, including
  // disappointmentSignal — this triggers the hard gate regardless of the raw score.
  await expect(page.getByRole("heading", { name: /^Your result: No fit yet/ })).toBeVisible();
  await expect(page.getByText("Capped — core signal missing")).toBeVisible();
...
test("keyboard-only visitor can complete the PMF Score Checker Tool", async ({ page }) => {
  await page.goto("/tools/product-market-fit-tracker");

  // Tab to and activate "Start checking fit".
  await page.getByRole("button", { name: "Start checking fit" }).focus();
...
test.describe("mobile viewport", () => {
  ...
  test("PMF Score Checker Tool is completable on a mobile viewport", async ({ page }) => {
    await page.goto("/tools/product-market-fit-tracker");
    await page.getByRole("button", { name: "Start checking fit" }).click();
