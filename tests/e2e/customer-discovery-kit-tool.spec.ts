...
// generalised to the second Tool, plus keyboard-only and mobile-viewport completion).
...
test("anonymous visitor can complete the Evidence Analyser Tool and see a result", async ({ page }) => {
  await page.goto("/tools/customer-discovery-kit");
  await expect(page.getByRole("heading", { name: "Evidence Analyser", exact: false }).first()).toBeVisible();
  ...
  await expect(page.getByText(/Evidence strength score/)).toBeVisible();
  await expect(page.getByText(/Next evidence action/)).toBeVisible();
  // First option in every step is the weakest ("fewer_than_3" / "mostly_leading" / etc.), so
  // this run should land on the high-bias-risk, capped-score path.
  await expect(page.getByText("High bias risk")).toBeVisible();
...
test("keyboard-only visitor can complete the Evidence Analyser Tool", async ({ page }) => {
  await page.goto("/tools/customer-discovery-kit");

  // Tab to and activate "Start the analysis".
  await page.getByRole("button", { name: "Start the analysis" }).focus();
...
test.describe("mobile viewport", () => {
  ...
  test("Evidence Analyser Tool is completable on a mobile viewport", async ({ page }) => {
    await page.goto("/tools/customer-discovery-kit");
    await page.getByRole("button", { name: "Start the analysis" }).click();
